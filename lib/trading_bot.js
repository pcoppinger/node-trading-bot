...
class TradingBot {
     ...
     * @param options.auth {object} Auth details.
    ...
    constructor(options) {
        ...
        this.api_url = options.auth.apiURI;
        this.client = new gdax.AuthenticatedClient(this.auth.key, this.auth.secret, this.auth.passphrase, this.auth.apiURI);
        assert(this.client);
    ...
    get synced_book() {
        if (!this._orderbookSync) {
            this._orderbookSync = new gdax.OrderbookSync(this.product, this.api_url, this.websocket_url, this.auth);
            this.listen_to_messages();
        }
        return this._orderbookSync.books[this.product];
    }
    ...
    get order_book() {
        ...
        return this._orderbookSync.books[this.product].state();
    }
